...
export default function FontSize(){
    ...
    const { changeFontSize, size } = useTheme()
    ...
    return (
        <div className={"flex gap-4"}>
            <span className={`${size === 0 ? "text-gray-600" : 'cursor-pointer text-green-800'}`}  onClick={() => handleClick('decrease')}>
                <svg xmlns="http://www.w3.org/2000/svg" fill={"currentColor"} height="28" width="28" viewBox="0 0 48 48">
                    <path d="m2 38 11-28h4l11 28h-3.75l-2.85-7.5H8.6L5.75 38Zm7.8-10.7h10.4l-5.1-13.55h-.2Zm20.7-1.8v-3H46v3Z"/>
                </svg>
            </span>
            <span className={`${size === 4 ? " text-gray-600" : 'text-red-800 cursor-pointer'}`} onClick={() => handleClick('increase')}>
                <svg xmlns="http://www.w3.org/2000/svg" fill={"currentColor"} height="28" width="28" viewBox="0 0 48 48">
